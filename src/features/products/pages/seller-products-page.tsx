        ...
        <div className="flex flex-col gap-4 rounded-2xl border bg-card p-6 md:flex-row md:items-center md:justify-between">
          <div className="space-y-3">
            <h1 className="font-heading text-3xl font-semibold text-foreground md:text-4xl">
              {seller?.store_name ?? t("sellerProfile.fallbackTitle")}
            </h1>
            <p className="max-w-2xl text-sm leading-6 text-muted-foreground md:text-base">
              {seller?.description ?? t("sellerProfile.fallbackDescription")}
            </p>
            <div className="space-y-1 text-sm text-muted-foreground">
              {seller?.store_owner_name ? (
                <p>
                  {t("sellerProfile.owner", { name: seller.store_owner_name })}
                </p>
              ) : null}
