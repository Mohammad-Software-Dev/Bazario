...
import type { Ad, AdTargetType, AdTier, AdViewModel } from '@/features/ads/types/ad.types'
...
export function normalizeAdTargetType(adableType: string | null | undefined): AdTargetType | null {
  if (!adableType) {
    return null
  }
...
export function normalizeAdTier(positionName: string | null | undefined): AdTier | null {
  if (!positionName) {
    return null
  }
...
function getAdImageUrl(ad: Ad): string | null {
  const firstImage = ad.images[0] as (typeof ad.images)[number] & { path?: string | null } | undefined
...
function getOwnerName(ad: Ad): string | null {
  ...
  if (targetType === 'service' && ('serviceProvider' in target || 'service_provider' in target)) {
    const provider = ('service_provider' in target ? target.service_provider : target.serviceProvider) ?? null
