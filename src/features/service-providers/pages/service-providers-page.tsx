  ...
  return (
    <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-4 py-10 md:py-12">
      <section className="space-y-3">
        <h1 className="font-heading text-3xl font-semibold text-foreground md:text-4xl">{t('common.serviceProviders')}</h1>
      </section>

      {serviceProvidersQuery.isLoading ? <ServiceProvidersGridSkeleton /> : null}

      {!serviceProvidersQuery.isLoading && serviceProvidersQuery.isError ? (
        <Card>
          <CardContent className="py-6 text-sm text-destructive">
            {getApiErrorMessage(serviceProvidersQuery.error, t('catalog.loadServiceProvidersError'))}
          </CardContent>
        </Card>
      ) : null}
          ...
          {serviceProviders.length ? (
            <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
              {serviceProviders.map((serviceProvider) => (
                <ServiceProviderPreviewCard key={serviceProvider.id} serviceProvider={serviceProvider} />
              ))}
            </div>
          ) : (
            <Card>
              <CardContent className="py-6 text-sm text-muted-foreground">{t('home.emptyServices')}</CardContent>
            </Card>
          )}
