    ...
    >
      ...
      <CardContent className="space-y-4 p-4">
        <div className="space-y-3">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 space-y-1">
              <h3 className="line-clamp-2 text-lg font-semibold text-foreground">{serviceTitle}</h3>
              <p className="line-clamp-2 text-sm text-muted-foreground">{serviceDescription}</p>
            </div>
            {service.isNew ? (
        ...
        <div className="rounded-lg bg-muted/50 px-3 py-2.5 text-sm">
          ...
          <p className="line-clamp-1 text-muted-foreground">by {providerUserName}</p>
        </div>
      </CardContent>
    </Card>
  )
