...
export function MyListingCard({ listing }: MyListingCardProps) {
  ...
  async function handleCompletePayment() {
    ...
    try {
      ...
      setCheckoutError(
        getApiErrorMessage(error, t("listings.checkoutSessionError")),
      );
      ...
      <CardContent className="flex flex-col gap-4 p-4 md:flex-row md:items-start">
        ...
        <div className="min-w-0 flex-1 space-y-3">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-2 text-sm text-muted-foreground">
            ...
            <span>
              {t("listings.createdOn", {
                date: formatDateTime(listing.created_at, {
                  dateStyle: "medium",
                }),
              })}
            </span>
            ...
            <div className="space-y-2">
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  disabled={
                    isStartingCheckout || deleteListingMutation.isPending
                  }
                >
                  {isStartingCheckout
                    ? t("listings.startingCheckout")
                    : t("listings.completePayment")}
                </Button>
                <Button
                  type="button"
                  variant="outline"
