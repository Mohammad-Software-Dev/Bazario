    ...
    <header className="border-b bg-background">
      <div className="mx-auto flex min-h-16 w-full max-w-6xl items-center justify-between gap-4 px-4">
        <div className="flex items-center gap-6">
          ...
          <nav className="hidden items-center gap-1 lg:flex" aria-label={t('header.primaryNavigation')}>
        ...
        <div className="flex items-center gap-2 lg:hidden">
          <Button type="button" variant="ghost" size="icon" onClick={handleOpenChat} aria-label={t('header.chat')}>
            <MessageCircle className="size-5" />
            {isAuthenticated && unreadConversationCount > 0 ? (
              <span className="absolute mt-[-1.25rem] ml-[1.25rem] rounded-full bg-foreground px-1.5 py-0.5 text-[10px] font-medium text-background">
                {unreadConversationCount}
              </span>
            ) : null}
          </Button>
      ...
      <Dialog open={isMobileNavOpen} onOpenChange={setMobileNavOpen}>
        <DialogContent className="top-0 right-0 left-auto h-full max-h-none w-[min(22rem,100%-1rem)] translate-x-0 translate-y-0 rounded-none border-l border-t-0 border-r-0 border-b-0 p-0">
          ...
          <div className="flex h-full flex-col overflow-y-auto px-5 py-5">
            <nav className="space-y-1" aria-label={t('header.primaryNavigation')}>
              {navigationItems.map((item) => (
                <Button key={item.to} asChild variant="ghost" className="w-full justify-start px-3">
            ...
            <div className="mt-6 space-y-2 border-t border-border/70 pt-6">
              {isAuthenticated ? (
                <>
                  <Button asChild variant="ghost" className="w-full justify-start px-3">
